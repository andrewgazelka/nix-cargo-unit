...
impl UnitGraph {
    ...
    /// Temporary Nix generation (to be refactored in feature 5).
    pub fn to_nix(&self) -> String {
        ...
        for (i, unit) in self.units.iter().enumerate() {
            let crate_type = unit
                ...
                .crate_types
                .first()
                .map_or("lib", |s| s.as_str());
            let features_nix: Vec<String> =
                unit.features.iter().map(|f| format!("\"{f}\"")).collect();
